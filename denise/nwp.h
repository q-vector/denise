//
// nwp.h
// 
// Copyright (C) 2005-2013 Simon E. Ching
// 
// This file is part of libdenise.
//
// libdenise is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// libdenise is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with libdenise.  If not, see <http://www.gnu.org/licenses/>.

#ifndef DENISE_NWP_H
#define DENISE_NWP_H

#include <denise/gis.h>
#include <denise/geodesy.h>
#include <denise/thermo.h>

using namespace std;

#define TIME_TOLERANCE 0.1

namespace denise
{

   enum Nwp_Element
   {
      PRESSURE,
      TEMPERATURE,
      TEMPERATURE_CELCIUS,
      LAPSE,
      MIX_DOWN_TEMPERATURE,
      ZONAL_WIND,
      MERIDIONAL_WIND,
      WIND_SPEED,
      WIND_DIRECTION,
      VERTICAL_VELOCITY,
      STREAMLINE_WIND,
      NORMAL_WIND,
      GEOPOTENTIAL_HEIGHT,
      DEW_POINT,
      MEAN_SEA_LEVEL_PRESSURE,
      HIGH_CLOUD,
      MIDDLE_CLOUD,
      LOW_CLOUD,
      TOTAL_CLOUD,
      OMEGA,
      SNOW_LEVEL,
      FFDI,
      GFDI,
      CONTINUOUS_HAINES,
      PPT3,
      PPT6,
      PPTN,
      RAINFALL_STEP,
      RAINFALL_CUMULATIVE,
      RELATIVE_HUMIDITY,
      DEW_POINT_DEPRESSION,
      POTENTIAL_TEMPERATURE,
      THETA,
      THETA_E,
      THETA_W,
      MIXING_RATIO,
      MONTGOMERY,
      SLI,
      SHOWALTER,
      LI_700,
      LI_THUNDER,
      K_INDEX,
      TOTAL_TOTALS,
      CAPE,
      PRECIPITABLE_WATER,
      FOG_FRACTION,
      THICKNESS,
      POTENTIAL_VORTICITY,
      PV1_5_PRESSURE,
      ABSOLUTE_VORTICITY,
      SHEAR_VORTICITY,
      CURVATURE_VORTICITY,
      TEMPERATURE_ADVECTION,
      ADIABATIC_HEATING,
      LATENT_HEATING,
      VARIANCE_TEMPERATURE,
      VARIANCE_RELATIVE_HUMIDITY,
      VARIANCE_GEOPOTENTIAL_HEIGHT,
      VARIANCE_ZONAL_WIND,
      VARIANCE_MERIDIONAL_WIND,
      VARIANCE_OMEGA
   };

   class Level_Tuple : public Tuple
   {

      public:

         const Level_Type
         type;

         //Level_Tuple ();

         Level_Tuple (const Level_Type type,
                      const string& str,
                      const string& delimiter = ":");

         void
         add (const string& str,
              const string& delimiter = ":",
              const bool clear_first = false);

         const Real
         get_next_up (const Real value) const;

         const Real
         get_next_down (const Real value) const;

   };

   class Level_Element
   {

      public:

         Level
         level;

         Nwp_Element
         nwp_element;

         Level_Element (const Level& level,
                        const Nwp_Element nwp_element);

   };

   class Nwp
   {

      public:

         class Key
         {

            public:

               Dtime
               base_time;

               Integer
               forecast_hour;

               Key () { }

               Key (const Dtime& base_time,
                    const Integer forecast_hour = 0);

               Key (const Key& key);

               bool
               operator == (const Key& key) const;

               bool
               operator > (const Key& key) const;

               bool
               operator < (const Key& key) const;

         };

         class Sounding : public denise::Sounding
         {

            protected:

               const Key&
               key;

            public:

               Sounding (const Key& key);

         };

         class Element_Vector : public vector<Nwp_Element>
         {

             private:

                map<Nwp_Element, Integer>
                reverse_map;

             public:

                Element_Vector (const vector<Nwp_Element>& nwp_element_vector);

                Integer
                get_index (const Nwp_Element nwp_element) const;

                Nwp_Element
                get_nwp_element (const Integer element_index) const;

         };

         // Assumes vertical coordinate is p
         class Data_3D : public map<Nwp_Element, Geodetic_Vector_Data_3D*>
         {

            private:

               bool
               available;

            public:

               const Key
               key;

               Data_3D (const vector<Nwp_Element>& nwp_element_vector,
                        const Key& key);

               ~Data_3D ();

               void
               set_gvd_3d_ptr (const Nwp_Element nwp_element,
                               Geodetic_Vector_Data_3D* gvd_3d_ptr);

               const Geodetic_Vector_Data_3D&
               get_gvd_3d (const Nwp_Element nwp_element) const;

               Geodetic_Vector_Data_3D&
               get_gvd_3d (const Nwp_Element nwp_element);

               virtual const Tuple&
               get_tuple_p (const Nwp_Element nwp_element) const;

               Real
               get_p (const Nwp_Element nwp_element,
                      const Integer k) const;

               Lat_Long
               get_lat_long (const Nwp_Element nwp_element,
                             const Integer i,
                             const Integer j) const;

               bool
               is_available () const;

               void
               set_available ();

               void
               initialize (const Real datum);

               virtual Real
               get_p_from_element (const Nwp_Element nwp_element,
                                   const Integer i,
                                   const Integer j,
                                   const Real element_value) const;

               virtual Real
               get_p_from_element (const Nwp_Element nwp_element,
                                   const Real latitude,
                                   const Real longitude,
                                   const Real element_value) const;

               virtual Real
               evaluate (const Nwp_Element element,
                         const Integer k,
                         const Integer i,
                         const Integer j,
                         const Evaluate_Op evaluate_op = VALUE) const;

               virtual Real
               evaluate (const Nwp_Element element,
                         const Real p,
                         const Integer i,
                         const Integer j,
                         const Evaluate_Op evaluate_op = VALUE) const;

               virtual Real
               evaluate (const Nwp_Element element,
                         const Integer k,
                         const Real latitude,
                         const Real longitude,
                         const Evaluate_Op evaluate_op = VALUE) const;

               virtual Real
               evaluate (const Nwp_Element element,
                         const Integer k,
                         const Lat_Long& lat_long,
                         const Evaluate_Op evaluate_op = VALUE) const;

               virtual Real
               evaluate (const Nwp_Element element,
                         const Real p,
                         const Real latitude,
                         const Real longitude,
                         const Evaluate_Op evaluate_op = VALUE) const;

               virtual Real
               evaluate (const Nwp_Element element,
                         const Real p,
                         const Lat_Long& lat_long,
                         const Evaluate_Op evaluate_op = VALUE) const;

               Real
               get_li_thunder (const Integer k,
                               const Integer i,
                               const Integer j,
                               const Real thunder_p,
                               const Real thunder_t) const;

               Real
               get_li_thunder (const Real p,
                               const Integer i,
                               const Integer j,
                               const Real thunder_p,
                               const Real thunder_t) const;

               Real
               get_li_thunder (const Integer k,
                               const Real latitude,
                               const Real longitude,
                               const Real thunder_p,
                               const Real thunder_t) const;

               Real
               get_li_thunder (const Integer k,
                               const Lat_Long& lat_long,
                               const Real thunder_p,
                               const Real thunder_t) const;

               Real
               get_li_thunder (const Real p,
                               const Real latitude,
                               const Real longitude,
                               const Real thunder_p,
                               const Real thunder_t) const;

               Real
               get_li_thunder (const Real p,
                               const Lat_Long& lat_long,
                               const Real thunder_p,
                               const Real thunder_t) const;

         };

         class Time_Cross : public map<Nwp_Element, Scalar_Data_2D*>
         {

            private:

               Scalar_Data_1D*
               terrain_profile_ptr;

               Scalar_Data_1D*
               rainfall_profile_ptr;

            public:

               Time_Cross ();

               ~Time_Cross ();

               void
               set_profile_ptrs (Scalar_Data_1D* terrain_profile_ptr,
                                 Scalar_Data_1D* rainfall_profile_ptr);

               void
               insert_nwp_element_if_needed (const Nwp_Element nwp_element,
                                             const Tuple& tuple_t,
                                             const Tuple& tuple_p);

               const Scalar_Data_1D&
               get_terrain_profile () const;

               const Scalar_Data_1D&
               get_rainfall_profile () const;

               const Scalar_Data_2D&
               get_sd_2d (const Nwp_Element nwp_element) const;

               Scalar_Data_2D&
               get_sd_2d (const Nwp_Element nwp_element);

               virtual const Tuple&
               get_tuple_p (const Nwp_Element nwp_element) const;

         };

         class Cross_Section : public map<Nwp_Element, Scalar_Data_2D*>
         {

            private:

               Scalar_Data_1D*
               terrain_profile_ptr;

               Scalar_Data_1D*
               rainfall_profile_ptr;

            public:

               Cross_Section ();

               ~Cross_Section ();

               void
               set_profile_ptrs (Scalar_Data_1D* terrain_profile_ptr,
                                 Scalar_Data_1D* rainfall_profile_ptr);

               void
               insert_nwp_element_if_needed (const Nwp_Element nwp_element,
                                             const Tuple& tuple_x,
                                             const Tuple& tuple_p);

               const Scalar_Data_1D&
               get_terrain_profile () const;

               const Scalar_Data_1D&
               get_rainfall_profile () const;

               const Scalar_Data_2D&
               get_sd_2d (const Nwp_Element nwp_element) const;

               Scalar_Data_2D&
               get_sd_2d (const Nwp_Element nwp_element);

               virtual const Tuple&
               get_tuple_p (const Nwp_Element nwp_element) const;

         };

         class Key_Multimap : public multimap<Dtime, Integer>
         {

            private:

               set<Key>
               key_set;

            public:

               void
               clear ();

               void
               add (const Key& key);

               bool
               is_no_match (const Key& key) const;

               bool
               is_first_step (const Key& key) const;

               Key
               get_previous_key (const Key& key) const;

               set<Dtime>
               get_base_time_set () const;

               set<Dtime>
               get_valid_time_set (const Dtime& base_time) const;

               Key
               get_key (const Dtime& dtime) const;

               Key
               get_key (const Dtime& dtime,
                        const Dtime& base_time) const;

         };

      protected:

         vector<Nwp_Element>
         nwp_element_vector;

         const string
         path;

         const string
         description;

         string
         status;

         Key_Multimap
         key_multimap;

         Tuple
         tuple_p;

         map<Key, Data_3D*>
         data_3d_ptr_map;

         virtual Geodetic_Vector_Data_2D*
         get_initialized_vd_2d (const Integer vector_size) const = 0;

         virtual void
         initialize_3d_data (const Key& key) = 0;

         virtual void
         load_3d_data (Data_3D& data_3d) = 0;

         virtual void
         fill_lapse_data (Geodetic_Vector_Data_2D& gvd_2d,
                          const Integer vector_index,
                          const Key& key,
                          const Level& level);

         virtual void
         fill_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Key& key,
                         const Nwp_Element nwp_element);

         virtual void
         fill_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Key& key,
                         const Integer hours = -1);

         void
         fill_p_potential_vorticity_data (Geodetic_Vector_Data_2D& gvd_2d,
                                          const Integer vector_index,
                                          const Key& key,
                                          const Real p);

         void
         fill_theta_potential_vorticity_data (Geodetic_Vector_Data_2D& gvd_2d,
                                              const Integer vector_index,
                                              const Key& key,
                                              const Real theta);

         void
         fill_potential_vorticity_data (Geodetic_Vector_Data_2D& gvd_2d,
                                        const Integer vector_index,
                                        const Key& key,
                                        const Level& level);

         void
         fill_p_temperature_advection_data (Geodetic_Vector_Data_2D& gvd_2d,
                                            const Integer vector_index,
                                            const Key& key,
                                            const Real p);

         void
         fill_theta_temperature_advection_data (Geodetic_Vector_Data_2D& gvd_2d,
                                                const Integer vector_index,
                                                const Key& key,
                                                const Real theta);

         void
         fill_temperature_advection_data (Geodetic_Vector_Data_2D& gvd_2d,
                                          const Integer vector_index,
                                          const Key& key,
                                          const Level& level);

         void
         fill_p_adiabatic_heating_data (Geodetic_Vector_Data_2D& gvd_2d,
                                        const Integer vector_index,
                                        const Key& key,
                                        const Real p);

         void
         fill_theta_adiabatic_heating_data (Geodetic_Vector_Data_2D& gvd_2d,
                                            const Integer vector_index,
                                            const Key& key,
                                            const Real theta);

         void
         fill_adiabatic_heating_data (Geodetic_Vector_Data_2D& gvd_2d,
                                      const Integer vector_index,
                                      const Key& key,
                                      const Level& level);

         void
         fill_p_latent_heating_data (Geodetic_Vector_Data_2D& gvd_2d,
                                     const Integer vector_index,
                                     const Key& key,
                                     const Real p);

         void
         fill_theta_latent_heating_data (Geodetic_Vector_Data_2D& gvd_2d,
                                         const Integer vector_index,
                                         const Key& key,
                                         const Real theta);

         void
         fill_latent_heating_data (Geodetic_Vector_Data_2D& gvd_2d,
                                   const Integer vector_index,
                                   const Key& key,
                                   const Level& level);

         virtual void
         fill_pv_p_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Key& key,
                         const Real pv_threshold = 1.5e-6);

         void
         fill_absolute_vorticity_data (Geodetic_Vector_Data_2D& gvd_2d,
                                       const Integer vector_index,
                                       const Key& key,
                                       const Level& level);

         void
         fill_shear_vorticity_data (Geodetic_Vector_Data_2D& gvd_2d,
                                    const Integer vector_index,
                                    const Key& key,
                                    const Level& level);

         void
         fill_curvature_vorticity_data (Geodetic_Vector_Data_2D& gvd_2d,
                                        const Integer vector_index,
                                        const Key& key,
                                        const Level& level);

         void
         fill_wind_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index_u,
                         const Integer vector_index_v,
                         const Key& key,
                         const Level& level);

         void
         fill_mix_down_temperature_data (Geodetic_Vector_Data_2D& gvd_2d,
                                         const Integer vector_index,
                                         const Key& key,
                                         const Level& level);

         virtual void
         fill_snow_probability_data (Geodetic_Vector_Data_2D& gvd_2d,
                                     const Integer vector_index,
                                     const Key& key);

         virtual void
         fill_snow_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                               const Integer vector_index,
                               const Key& key,
                               const Real probability);

         virtual void
         fill_fdi_data (Geodetic_Vector_Data_2D& gvd_2d,
                        const Integer vector_index,
                        const Key& key,
                        const Level& level,
                        const Nwp_Element nwp_element);

         virtual void
         fill_continuous_haines_data (Geodetic_Vector_Data_2D& gvd_2d,
                                      const Integer vector_index,
                                      const Key& key);

         virtual void
         fill_total_totals_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key);

         virtual void
         fill_k_index_data (Geodetic_Vector_Data_2D& gvd_2d,
                            const Integer vector_index,
                            const Key& key);

         virtual void
         fill_lifted_index_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Real bottom_p,
                                 const Real top_p);

         virtual void
         fill_surface_lifted_index_data (Geodetic_Vector_Data_2D& gvd_2d,
                                         const Integer vector_index,
                                         const Key& key,
                                         const Real top_p);

         virtual void
         fill_li_thunder_data (Geodetic_Vector_Data_2D& gvd_2d,
                               const Integer vector_index,
                               const Key& key,
                               const Level& level,
                               const Real thunder_t = -20 + K);

         virtual void
         fill_ts_diagnosis_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Level& level,
                                 const Nwp_Element nwp_element);

         virtual void
         fill_cloud_data (Geodetic_Vector_Data_2D& gvd_2d,
                          const Integer vector_index,
                          const Key& key,
                          const Nwp_Element nwp_element);

         virtual void
         fill_pressure_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                   const Integer vector_index,
                                   const Key& key,
                                   const Real p,
                                   const Nwp_Element nwp_element);

         virtual void
         fill_theta_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                const Integer vector_index,
                                const Key& key,
                                const Real theta,
                                const Nwp_Element nwp_element);

         virtual void
         fill_sigma_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                const Integer vector_index,
                                const Key& key,
                                const Real sigma,
                                const Nwp_Element nwp_element,
                                const Geodetic_Vector_Data_2D& surface_p_data);

         virtual void
         fill_screen_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Nwp_Element nwp_element);

         virtual void
         fill_50m_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key,
                              const Nwp_Element nwp_element);

         virtual void
         fill_10m_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key,
                              const Nwp_Element nwp_element);

         virtual void
         fill_msl_data (Geodetic_Vector_Data_2D& gvd_2d,
                        const Integer vector_index,
                        const Key& key,
                        const Nwp_Element nwp_element);

         virtual void
         fill_nil_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key,
                              const Nwp_Element nwp_element);

         virtual void
         fill_surface_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                  const Integer vector_index,
                                  const Key& key,
                                  const Nwp_Element nwp_element);

         virtual void
         fill_data (Geodetic_Vector_Data_2D& gvd_2d,
                    const Integer vector_index,
                    const Key& key,
                    const Level& level,
                    const Nwp_Element nwp_element);

      public:

         Nwp (const string& description,
              const string& path);

         ~Nwp ();

         virtual void
         survey () = 0;

         virtual void
         clean_up () = 0;

         void
         clear_data_3d_ptr_map ();

         virtual void
         set_domain_2d (const Domain_2D& domain_2d);

         const string&
         get_description () const;

         const string&
         get_status () const;

         virtual set<Dtime>
         get_valid_time_set (const Dtime& base_time) const;

         virtual Tuple
         get_valid_t_tuple (const Dtime& base_time,
                            const Dtime& start_time,
                            const Dtime& end_time) const;

         virtual set<Dtime>
         get_base_time_set () const;

         virtual Key
         get_key (const Dtime& dtime,
                  const Dtime& base_time) const;

         Nwp::Data_3D&
         get_3d_data (const Key& key);

         virtual Geodetic_Vector_Data_2D*
         get_ts_steering_data_ptr (const Key& key);

         virtual Geodetic_Vector_Data_2D*
         get_steering_data_ptr (const Key& key,
                                const Layer& layer);

         virtual Geodetic_Vector_Data_2D*
         get_vertical_shear_data_ptr (const Key& key,
                                      const Layer& layer);

         virtual Geodetic_Vector_Data_2D*
         get_freezing_level_data_ptr (const Key& key);

         virtual Geodetic_Vector_Data_2D*
         get_temperature_p_data_ptr (const Key& key,
                                     const Real temperature);

         virtual Geodetic_Vector_Data_2D*
         get_p_data_ptr (const Key& key,
                         const Level& level);

         virtual Geodetic_Vector_Data_2D*
         get_next_p_up_data_ptr (const Key& key,
                                 const Level& level);

         virtual Geodetic_Vector_Data_2D*
         get_cloud_data_ptr (const Key& key);

         virtual Geodetic_Vector_Data_2D*
         get_cloud_base_data_ptr (const Key& key,
                                  const Level& level);

         virtual Geodetic_Vector_Data_2D*
         get_cloud_top_temp_data_ptr (const Key& key);

         virtual Geodetic_Vector_Data_2D*
         get_ageostrophic_wind_data_ptr (const Key& key,
                                         const Real p);

         virtual Geodetic_Vector_Data_2D*
         get_surface_ageostrophic_wind_data_ptr (const Key& key,
                                                 const Level& level);

         virtual Geodetic_Vector_Data_2D*
         get_ageostrophic_wind_data_ptr (const Key& key,
                                         const Level& level);

         virtual Geodetic_Vector_Data_2D*
         get_geostrophic_wind_data_ptr (const Key& key,
                                        const Real p);

         virtual Geodetic_Vector_Data_2D*
         get_surface_geostrophic_wind_data_ptr (const Key& key);

         virtual Geodetic_Vector_Data_2D*
         get_geostrophic_wind_data_ptr (const Key& key,
                                        const Level& level);

         virtual Geodetic_Vector_Data_2D*
         get_wind_data_ptr (const Key& key,
                            const Level& level);

         virtual Geodetic_Vector_Data_2D*
         get_horizontal_shear_data_ptr (const Key& key,
                                        const Level& level,
                                        const bool with_wind = false);

         virtual Geodetic_Vector_Data_2D*
         get_omega_data_ptr (const Key& key,
                             const Level& level,
                             const bool with_wind = false);

         virtual Geodetic_Vector_Data_2D*
         get_temperature_24hr_data_ptr (const Key& key,
                                        const Level& level);

         virtual Geodetic_Vector_Data_2D*
         get_mix_down_temperature_data_ptr (const Key& key,
                                            const Level& level,
                                            const bool with_wind = false);

         virtual Geodetic_Vector_Data_2D*
         get_stratus_data_ptr (const Key& key,
                               const Level& level,
                               const bool with_wind = false);

         virtual Geodetic_Vector_Data_2D*
         get_lapse_data_ptr (const Key& key,
                             const Level& level,
                             const bool with_wind = false);

         virtual Geodetic_Vector_Data_2D*
         get_temperature_data_ptr (const Key& key,
                                   const Level& level,
                                   const bool with_wind = false);

         virtual Geodetic_Vector_Data_2D*
         get_dew_point_data_ptr (const Key& key,
                                 const Level& level,
                                 const bool with_wind = false);

         virtual Geodetic_Vector_Data_2D*
         get_rh_data_ptr (const Key& key,
                          const Level& level,
                          const bool with_wind = false);

         virtual Geodetic_Vector_Data_2D*
         get_dew_point_depression_data_ptr (const Key& key,
                                            const Level& level,
                                            const bool with_wind = false);

         virtual Geodetic_Vector_Data_2D*
         get_theta_e_data_ptr (const Key& key,
                               const Level& level,
                               const bool with_wind = false);

         virtual Geodetic_Vector_Data_2D*
         get_theta_w_data_ptr (const Key& key,
                               const Level& level,
                               const bool with_wind = false);

         virtual Geodetic_Vector_Data_2D*
         get_q_vector_data_ptr (const Key& key,
                                const Level& level);

         virtual Geodetic_Vector_Data_2D*
         get_temperature_advection_data_ptr (const Key& key,
                                             const Level& level);

         virtual Geodetic_Vector_Data_2D*
         get_gd_sea_wave_data_ptr (const Key& key,
                                   const Level& level,
                                   const Fetch& fetch,
                                   const Real max_fetch = 150e3);

         virtual Geodetic_Vector_Data_2D*
         get_step_rainfall_data_ptr (const Key& key);

         virtual Geodetic_Vector_Data_2D*
         get_rainfall_data_ptr (const Key& key,
                                const Integer hours = -1);

         virtual Geodetic_Vector_Data_2D*
         get_sutcliffe_data_ptr (const Key& key);

         virtual Geodetic_Vector_Data_2D*
         get_pv_p_data_ptr (const Key& key,
                            const Real pv_threshold = 1.5e-6);

         virtual Geodetic_Vector_Data_2D*
         get_theta_level_data_ptr (const Key& key,
                                   const Real theta,
                                   const vector<Nwp_Element> element_vector,
                                   const bool with_pv);

         virtual Geodetic_Vector_Data_2D*
         get_sigma_level_data_ptr (const Key& key,
                                   const Real sigma,
                                   const vector<Nwp_Element> element_vector);

         virtual Geodetic_Vector_Data_2D*
         get_pressure_level_data_ptr (const Key& key,
                                      const Real p,
                                      const vector<Nwp_Element> element_vector);

         virtual Geodetic_Vector_Data_2D*
         get_data_ptr (const Key& key,
                       const Level& level,
                       const Nwp_Element nwp_element);

         virtual Geodetic_Vector_Data_2D*
         get_data_ptr (const Key& key,
                       const Level& level,
                       const vector<Nwp_Element>& nwp_element_vector);

         virtual Scalar_Data_1D*
         get_terrain_profile_ptr (const Key& key,
                                  const Multi_Journey& multi_journey);

         virtual Scalar_Data_1D*
         get_rainfall_profile_ptr (const Key& key,
                                   const Multi_Journey& multi_journey);

         virtual Cross_Section*
         get_cross_section_ptr (const Key& key,
                                const Multi_Journey& multi_journey,
                                const Nwp_Element nwp_element,
                                const bool with_wind);

         virtual Cross_Section*
         get_cross_section_ptr (const Key& key,
                                const Multi_Journey& multi_journey,
                                const vector<Nwp_Element>& element_vector,
                                const bool with_wind);

         virtual Nwp::Sounding*
         get_sounding_ptr (const Lat_Long& lat_long,
                           const Key& key);

         virtual Tuple
         get_point_tuple (const Lat_Long& lat_long,
                          const Key& key);

         virtual Vector_Data_1D*
         get_time_series_ptr (const Lat_Long& lat_long,
                              const Dtime& base_time,
                              const Dtime& start_time,
                              const Dtime& end_time,
                              const Level_Element level_element);

         virtual Vector_Data_1D*
         get_time_series_ptr (const Lat_Long& lat_long,
                              const Dtime& base_time,
                              const Dtime& start_time,
                              const Dtime& end_time,
                              const vector<Level_Element>& level_element_vector);

         virtual Time_Cross*
         get_time_cross_data_ptr (const Lat_Long& lat_long,
                                  const Dtime& base_time,
                                  const Dtime& start_time,
                                  const Dtime& end_time,
                                  const vector<Nwp_Element>& element_vector);

   };

   class Nwp_Exception : public Exception
   {

      public:

         Nwp_Exception (const string& str = string (""));

   };

   class Access : public Nwp,
                  public map<Grib::Key, Grib*>
   {

      private:

         class Data_3D : public Nwp::Data_3D
         {

            public:

               Data_3D (const vector<Nwp_Element>& nwp_element_vector,
                        const Key& key);

               virtual Real
               evaluate (const Nwp_Element element,
                         const Real p,
                         const Real latitude,
                         const Real longitude,
                         const Evaluate_Op evaluate_op = VALUE) const;


         };

         set<Grib*>
         grib_ptr_set;

         const bool
         omega_as_w;

         const string
         data_path;

         const string
         search_string;

         Size_2D
         size_2d;

         Domain_2D
         domain_2d;

         Grib::Key
         get_grib_key (const Key& key,
                       const Nwp_Element nwp_element,
                       const Level& level) const;

         void
         set_grib_key (Grib::Key& grib_key,
                       const Nwp_Element nwp_element,
                       const Dtime& base_time,
                       const Integer forecast_hour) const;

         void
         set_grib_key (Grib::Key& grib_key,
                       const Nwp_Element nwp_element,
                       const denise::Level& level) const;

         void
         initialize_3d_data (const Key& key);

         void
         load_3d_data (Nwp::Data_3D& data_3d);

         Geodetic_Vector_Data_2D*
         get_initialized_vd_2d (const Integer vector_size) const;

         void
         fill_ts_diagnosis_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Level& level,
                                 const Nwp_Element nwp_element);

         void
         fill_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Key& key,
                         const Nwp_Element nwp_element);

         void
         fill_cloud_data (Geodetic_Vector_Data_2D& gvd_2d,
                          const Integer vector_index,
                          const Key& key,
                          const Nwp_Element nwp_element);

         void
         fill_screen_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Nwp_Element nwp_element);

         void
         fill_10m_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key,
                              const Nwp_Element nwp_element);

         void
         fill_msl_data (Geodetic_Vector_Data_2D& gvd_2d,
                        const Integer vector_index,
                        const Key& key,
                        const Nwp_Element nwp_element);

         void
         fill_surface_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                  const Integer vector_index,
                                  const Key& key,
                                  const Nwp_Element nwp_element);

         Geodetic_Vector_Data_3D*
         get_gvd_3d_ptr (const Nwp_Element nwp_element,
                         const Key& key) const;

         Geodetic_Vector_Data_2D*
         get_grib_data_ptr (const Grib& grib,
                            const Grib::Key& grib_key) const;

         void
         fill_grib_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Nwp_Element nwp_element,
                         const Key& key,
                         const Level& level) const;

      public:

         Access (const string& description,
                 const string& data_path,
                 const string& search_string,
                 const bool omega_as_w = false);

         ~Access ();

         void
         survey ();

         void
         clean_up ();

   };

   class Ecmwf : public Nwp,
                 public map<Grib::Key, Grib*>
   {

      private:

         class Data_3D : public Nwp::Data_3D
         {

            public:

               Data_3D (const vector<Nwp_Element>& nwp_element_vector,
                        const Key& key);

               virtual Real
               evaluate (const Nwp_Element element,
                         const Real p,
                         const Real latitude,
                         const Real longitude,
                         const Evaluate_Op evaluate_op = VALUE) const;


         };

         set<Grib*>
         grib_ptr_set;

         const bool
         omega_as_w;

         const string
         data_path;

         const string
         search_string;

         map<Real, Size_2D>
         size_2d_map;

         Domain_2D
         domain_2d;

         Grib::Key
         get_grib_key (const Key& key,
                       const Nwp_Element nwp_element,
                       const Level& level) const;

         void
         set_grib_key (Grib::Key& grib_key,
                       const Nwp_Element nwp_element,
                       const Dtime& base_time,
                       const Integer forecast_hour) const;

         void
         set_grib_key (Grib::Key& grib_key,
                       const Nwp_Element nwp_element,
                       const denise::Level& level) const;

         void
         initialize_3d_data (const Key& key);

         void
         load_3d_data (Nwp::Data_3D& data_3d);

         Geodetic_Vector_Data_2D*
         get_initialized_vd_2d (const Integer vector_size) const;

         void
         fill_ts_diagnosis_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Level& level,
                                 const Nwp_Element nwp_element);

         void
         fill_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Key& key,
                         const Nwp_Element nwp_element);

         void
         fill_cloud_data (Geodetic_Vector_Data_2D& gvd_2d,
                          const Integer vector_index,
                          const Key& key,
                          const Nwp_Element nwp_element);

         void
         fill_screen_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Nwp_Element nwp_element);

         void
         fill_10m_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key,
                              const Nwp_Element nwp_element);

         void
         fill_msl_data (Geodetic_Vector_Data_2D& gvd_2d,
                        const Integer vector_index,
                        const Key& key,
                        const Nwp_Element nwp_element);

         void
         fill_surface_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                  const Integer vector_index,
                                  const Key& key,
                                  const Nwp_Element nwp_element);

         Geodetic_Vector_Data_3D*
         get_gvd_3d_ptr (const Nwp_Element nwp_element,
                         const Key& key) const;

         Geodetic_Vector_Data_2D*
         get_grib_data_ptr (const Grib& grib,
                            const Grib::Key& grib_key) const;

         void
         fill_grib_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Nwp_Element nwp_element,
                         const Key& key,
                         const Level& level) const;

      public:

         Ecmwf (const string& description,
                const string& data_path,
                const string& search_string,
                const bool omega_as_w = false);

         ~Ecmwf ();

         void
         survey ();

         void
         clean_up ();

   };

   class Gfs3 : public Nwp,
                public map<Nwp::Key, Grib*>
   {

      private:

         class Data_3D : public Nwp::Data_3D
         {

            public:

               Data_3D (const vector<Nwp_Element>& nwp_element_vector,
                        const Key& key);

               virtual Real
               evaluate (const Nwp_Element element,
                         const Real p,
                         const Real latitude,
                         const Real longitude,
                         const Evaluate_Op evaluate_op = VALUE) const;


         };

         const string
         data_path;

         Size_2D
         size_2d;

         Domain_2D
         domain_2d;

         Grib::Key
         get_grib_key (const Key& key,
                       const Nwp_Element nwp_element,
                       const Level& level) const;

         void
         set_grib_key (Grib::Key& grib_key,
                       const Dtime& base_time) const;

         void
         set_grib_key (Grib::Key& grib_key,
                       const Nwp_Element nwp_element,
                       const Integer forecast_hour) const;

         void
         set_grib_key (Grib::Key& grib_key,
                       const Nwp_Element nwp_element) const;

         void
         set_grib_key (Grib::Key& grib_key,
                       const Nwp_Element nwp_element,
                       const denise::Level& level) const;

         void
         initialize_3d_data (const Key& key);

         void
         load_3d_data (Nwp::Data_3D& data_3d);

         Geodetic_Vector_Data_2D*
         get_initialized_vd_2d (const Integer vector_size) const;

         void
         fill_ts_diagnosis_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Level& level,
                                 const Nwp_Element nwp_element);

         void
         fill_cumulative_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                                    const Integer vector_index,
                                    const Key& key);

         void
         fill_step_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key);

         void
         fill_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Key& key,
                         const Nwp_Element nwp_element);

         void
         fill_cloud_data (Geodetic_Vector_Data_2D& gvd_2d,
                          const Integer vector_index,
                          const Key& key,
                          const Nwp_Element nwp_element);

         void
         fill_screen_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Nwp_Element nwp_element);

         void
         fill_10m_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key,
                              const Nwp_Element nwp_element);

         void
         fill_msl_data (Geodetic_Vector_Data_2D& gvd_2d,
                        const Integer vector_index,
                        const Key& key,
                        const Nwp_Element nwp_element);

         void
         fill_surface_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                  const Integer vector_index,
                                  const Key& key,
                                  const Nwp_Element nwp_element);

/*
         void
         fill_grib_data (Geodetic_Vector_Data_3D& gvd_3d,
                         const Integer vector_index,
                         const Nwp_Element nwp_element,
                         const Key& key) const;
*/
         Geodetic_Vector_Data_3D*
         get_gvd_3d_ptr (const Nwp_Element nwp_element,
                         const Key& key) const;

         Geodetic_Vector_Data_2D*
         get_grib_data_ptr (const Grib& grib,
                            const Grib::Key& grib_key) const;

         void
         fill_grib_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Nwp_Element nwp_element,
                         const Key& key,
                         const Level& level) const;

      public:

         Gfs3 (const string& data_path);

         ~Gfs3 ();

         void
         survey ();

         void
         clear_3d_data ();

         void
         clean_up ();

         vector<Dtime>
         get_valid_time_vector () const;

         Key
         get_key (const Dtime& dtime) const;

         void
         acquire_base_time_forecast_hour (Dtime& base_time,
                                          Integer& forecast_hour,
                                          const Dtime& dtime) const;

   };

   class Gfs4 : public Nwp,
                public map<Nwp::Key, Grib2*>
   {

      private:

         class Data_3D : public Nwp::Data_3D
         {

            public:

               Data_3D (const vector<Nwp_Element>& nwp_element_vector,
                        const Key& key);

               virtual Real
               evaluate (const Nwp_Element element,
                         const Real p,
                         const Real latitude,
                         const Real longitude,
                         const Evaluate_Op evaluate_op = VALUE) const;


         };

         const string
         data_path;

         Size_2D
         grib_size_2d;

         Domain_2D
         grib_domain_2d;

         Size_2D
         size_2d;

         Domain_2D
         domain_2d;

         Grib2::Key
         get_grib_key (const Key& key,
                       const Nwp_Element nwp_element,
                       const Level& level) const;

         void
         set_grib_key (Grib2::Key& grib_key,
                       const Dtime& base_time) const;

         void
         set_grib_key (Grib2::Key& grib_key,
                       const Nwp_Element nwp_element,
                       const Integer forecast_hour) const;

         void
         set_grib_key (Grib2::Key& grib_key,
                       const Nwp_Element nwp_element) const;

         void
         set_grib_key (Grib2::Key& grib_key,
                       const Nwp_Element nwp_element,
                       const denise::Level& level) const;

         void
         initialize_3d_data (const Key& key);

         void
         load_3d_data (Nwp::Data_3D& data_3d);

         Geodetic_Vector_Data_2D*
         get_initialized_vd_2d (const Integer vector_size) const;

         void
         fill_ts_diagnosis_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Level& level,
                                 const Nwp_Element nwp_element);

         void
         fill_cumulative_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                                    const Integer vector_index,
                                    const Key& key);

         void
         fill_step_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key);

         void
         fill_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Key& key,
                         const Nwp_Element nwp_element);

         void
         fill_cloud_data (Geodetic_Vector_Data_2D& gvd_2d,
                          const Integer vector_index,
                          const Key& key,
                          const Nwp_Element nwp_element);

         void
         fill_screen_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Nwp_Element nwp_element);

         void
         fill_10m_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key,
                              const Nwp_Element nwp_element);

         void
         fill_msl_data (Geodetic_Vector_Data_2D& gvd_2d,
                        const Integer vector_index,
                        const Key& key,
                        const Nwp_Element nwp_element);

         void
         fill_surface_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                  const Integer vector_index,
                                  const Key& key,
                                  const Nwp_Element nwp_element);

/*
         void
         fill_grib_data (Geodetic_Vector_Data_3D& gvd_3d,
                         const Integer vector_index,
                         const Nwp_Element nwp_element,
                         const Key& key) const;
*/

         Geodetic_Vector_Data_3D*
         get_gvd_3d_ptr (const Nwp_Element nwp_element,
                         const Key& key) const;

         Geodetic_Vector_Data_2D*
         get_grib_data_ptr (const Grib2& grib,
                            const Grib2::Key& grib_key) const;

         void
         fill_grib_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Nwp_Element nwp_element,
                         const Key& key,
                         const Level& level) const;

      public:

         Gfs4 (const string& data_path);

         ~Gfs4 ();

         void
         survey ();

         void
         clean_up ();

         void
         set_domain_2d (const Domain_2D& domain_2d);

         vector<Dtime>
         get_valid_time_vector () const;

         Key
         get_key (const Dtime& dtime) const;

         void
         acquire_base_time_forecast_hour (Dtime& base_time,
                                          Integer& forecast_hour,
                                          const Dtime& dtime) const;

   };

   class Gfs : public Nwp,
               public map<Nwp::Key, Grib2*>
   {

      private:

         class Data_3D : public Nwp::Data_3D
         {

            public:

               Data_3D (const vector<Nwp_Element>& nwp_element_vector,
                        const Key& key);

               virtual Real
               evaluate (const Nwp_Element element,
                         const Real p,
                         const Real latitude,
                         const Real longitude,
                         const Evaluate_Op evaluate_op = VALUE) const;


         };

         const string
         data_path;

         Size_2D
         grib_size_2d;

         Domain_2D
         grib_domain_2d;

         Size_2D
         size_2d;

         Domain_2D
         domain_2d;

         Grib2::Key
         get_grib_key (const Key& key,
                       const Nwp_Element nwp_element,
                       const Level& level) const;

         void
         set_grib_key (Grib2::Key& grib_key,
                       const Dtime& base_time) const;

         void
         set_grib_key (Grib2::Key& grib_key,
                       const Nwp_Element nwp_element,
                       const Integer forecast_hour) const;

         void
         set_grib_key (Grib2::Key& grib_key,
                       const Nwp_Element nwp_element) const;

         void
         set_grib_key (Grib2::Key& grib_key,
                       const Nwp_Element nwp_element,
                       const denise::Level& level) const;

         void
         initialize_3d_data (const Key& key);

         void
         load_3d_data (Nwp::Data_3D& data_3d);

         Geodetic_Vector_Data_2D*
         get_initialized_vd_2d (const Integer vector_size) const;

         void
         fill_ts_diagnosis_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Level& level,
                                 const Nwp_Element nwp_element);

         void
         fill_cumulative_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                                    const Integer vector_index,
                                    const Key& key);

         void
         fill_step_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key);

         void
         fill_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Key& key,
                         const Nwp_Element nwp_element);

         void
         fill_cloud_data (Geodetic_Vector_Data_2D& gvd_2d,
                          const Integer vector_index,
                          const Key& key,
                          const Nwp_Element nwp_element);

         void
         fill_screen_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Nwp_Element nwp_element);

         void
         fill_10m_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key,
                              const Nwp_Element nwp_element);

         void
         fill_msl_data (Geodetic_Vector_Data_2D& gvd_2d,
                        const Integer vector_index,
                        const Key& key,
                        const Nwp_Element nwp_element);

         void
         fill_surface_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                  const Integer vector_index,
                                  const Key& key,
                                  const Nwp_Element nwp_element);

/*
         void
         fill_grib_data (Geodetic_Vector_Data_3D& gvd_3d,
                         const Integer vector_index,
                         const Nwp_Element nwp_element,
                         const Key& key) const;
*/

         Geodetic_Vector_Data_3D*
         get_gvd_3d_ptr (const Nwp_Element nwp_element,
                         const Key& key) const;

         Geodetic_Vector_Data_2D*
         get_grib_data_ptr (const Grib2& grib,
                            const Grib2::Key& grib_key) const;

         void
         fill_grib_data (Geodetic_Vector_Data_2D& gvd_2d,
                         const Integer vector_index,
                         const Nwp_Element nwp_element,
                         const Key& key,
                         const Level& level) const;

      public:

         Gfs (const string& data_path);

         ~Gfs ();

         void
         survey ();

         void
         clean_up ();

         void
         set_domain_2d (const Domain_2D& domain_2d);

         vector<Dtime>
         get_valid_time_vector () const;

         Key
         get_key (const Dtime& dtime) const;

         void
         acquire_base_time_forecast_hour (Dtime& base_time,
                                          Integer& forecast_hour,
                                          const Dtime& dtime) const;

   };

   class Ncep_Ncar_File
   {

      protected:

         int
         nc_id;

         const string&
         file_path;

         Integer
         get_dimension_size (const string& dimension_string) const;

      public:

         Ncep_Ncar_File (const string& file_path);

         ~Ncep_Ncar_File ();

         const Tuple&
         get_tuple_latitude () const;

         const Tuple&
         get_tuple_longitude () const;

   };

   class Ncep_Ncar_3d_File : public Ncep_Ncar_File
   {

      private:

         Integer
         actual_number_of_levels;

      public:

         Ncep_Ncar_3d_File (const string& file_path);

         void
         fill_data (Geodetic_Vector_Data_3D& data_3d,
                    const Integer vector_index,
                    const string& variable_string,
                    const Integer time_index) const;

         void
         fill_data (Geodetic_Vector_Data_2D& data_2d,
                    const Integer vector_index,
                    const string& variable_string,
                    const Integer time_index,
                    const Integer level_index) const;

   };

   class Ncep_Ncar_2d_File : public Ncep_Ncar_File
   {

      public:

         Ncep_Ncar_2d_File (const string& file_path);

         void
         fill_data (Geodetic_Vector_Data_2D& data_2d,
                    const Integer vector_index,
                    const string& variable_string,
                    const Integer time_index) const;

   };

   class Ncep_Ncar_Exception : public Exception
   {

      public:

         Ncep_Ncar_Exception (const string& description = string (""));

   }; 

   enum Element
   {
      U,
      V,
      OMG,
      T,
      RH,
      SH,
      Z,
      SURFACE_U,
      SURFACE_V,
      SURFACE_OMG,
      SURFACE_P,
      SURFACE_T,
      SURFACE_RH,
      SURFACE_THETA,
      PRECIP_WATER,
      SLP,
      U_10M,
      V_10M,
      T_2M,
      U_995,
      V_995,
      RH_995,
      T_995
   };

   class Ncep_Ncar : public Nwp
   {

      private:

         const Dtime
         start_time;

         const Real
         time_span;

         const Real
         time_interval;

         class Data_3D : public Nwp::Data_3D
         {

            public:

               Data_3D (const vector<Nwp_Element>& nwp_element_vector,
                        const Key& key);

               virtual Real
               evaluate (const Nwp_Element element,
                         const Real p,
                         const Real latitude,
                         const Real longitude,
                         const Evaluate_Op evaluate_op = VALUE) const;

         };

         const string
         data_path;

         Tuple
         tuple_latitude_gaussian;

         Tuple
         tuple_longitude_gaussian;

         Tuple
         tuple_p_omega;

         Tuple
         tuple_p_humid;

         string
         get_file_path (const Element element,
                        const Dtime& time) const;

         string
         get_element_string (const Element element) const;

         Integer
         get_time_index (const Dtime& time) const;

         Integer
         get_p_index (const Element element,
                      const Real p) const;

         const Tuple&
         get_tuple_p (const Element element) const;

         Geodetic_Vector_Data_2D*
         get_initialized_vd_2d (const Integer vector_size) const;

         void
         initialize_3d_data (const Key& key);

         void
         load_3d_data (Nwp::Data_3D& data_3d);

         void
         fill_cumulative_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                                    const Integer vector_index,
                                    const Key& key);

         void
         fill_step_rain_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key);

         void
         fill_screen_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Nwp_Element nwp_element);

         void
         fill_10m_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key,
                              const Nwp_Element nwp_element);

         void
         fill_msl_data (Geodetic_Vector_Data_2D& gvd_2d,
                        const Integer vector_index,
                        const Key& key,
                        const Nwp_Element nwp_element);

         void
         fill_surface_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                  const Integer vector_index,
                                  const Key& key,
                                  const Nwp_Element nwp_element);

         void
         fill_nc_data (Geodetic_Vector_Data_3D& gvd_3d,
                       const Integer vector_index,
                       const Element element,
                       const Dtime& time) const;

         void
         fill_nc_data (Geodetic_Vector_Data_2D& gvd_2d,
                       const Integer vector_index,
                       const Element element,
                       const Dtime& time,
                       const Real p = GSL_NAN) const;

         void
         init ();

      public:

         Ncep_Ncar (const string& data_path,
                    const Dtime& start_time,
                    const Real time_span,
                    const Real time_interval = 6);

         Ncep_Ncar (const string& data_path,
                    const Dtime& start_time,
                    const Dtime& end_time,
                    const Real time_interval = 6);

         void
         survey ();

         void
         clean_up ();

         static Tuple
         tuple_latitude ();

         static Tuple
         tuple_longitude ();

         const Tuple
         get_tuple_latitude () const;

         const Tuple
         get_tuple_longitude () const;

         set<Dtime>
         get_valid_time_set () const;

         Key
         get_key (const Dtime& dtime) const;

         void
         acquire_base_time_forecast_hour (Dtime& base_time,
                                          Integer& forecast_hour,
                                          const Dtime& dtime) const;

         Geodetic_Vector_Data_2D*
         get_surface_data_2d_ptr (const Key& key);

   };

   class Ncep_Ncar_Climate : public Nwp
   {

      private:

         class Data_3D : public Nwp::Data_3D
         {

            public:

               Data_3D (const vector<Nwp_Element>& nwp_element_vector,
                        const Key& key);

               virtual Real
               evaluate (const Nwp_Element element,
                         const Integer k,
                         const Integer i,
                         const Integer j,
                         const Evaluate_Op evaluate_op = VALUE) const;

               virtual Real
               evaluate (const Nwp_Element element,
                         const Real p,
                         const Integer i,
                         const Integer j,
                         const Evaluate_Op evaluate_op = VALUE) const;

               virtual Real
               evaluate (const Nwp_Element element,
                         const Integer k,
                         const Real latitude,
                         const Real longitude,
                         const Evaluate_Op evaluate_op = VALUE) const;

               virtual Real
               evaluate (const Nwp_Element element,
                         const Real p,
                         const Real latitude,
                         const Real longitude,
                         const Evaluate_Op evaluate_op = VALUE) const;

         };

         class Work_Data
         {

            private:

               Integer
               n;

               Geodetic_Vector_Data_2D*
               data_2d_ptr;

               Geodetic_Vector_Data_3D*
               data_3d_ptr;

               const Size_3D
               size_3d;

               void
               sum_up_2d (const Geodetic_Vector_Data_2D& nn_data);

               void
               sum_up_3d (Nwp::Data_3D& nn_data,
                          const Tuple& tuple_p);

               void
               divide_2d ();

               void
               divide_3d ();

            public:

               Work_Data (const Tuple& tuple_p,
                          const Tuple& tuple_latitude,
                          const Tuple& tuple_longitude);

               ~Work_Data ();

               void
               write (const string& path,
                      const Integer jjj) const;

               void
               sum_up (const Geodetic_Vector_Data_2D& nn_data_2d,
                       Nwp::Data_3D& nn_data_3d,
                       const Tuple& tuple_p);

               void
               divide ();

         };

         class Work_Data_Map : public map<Integer, Work_Data*>
         {

            private:

               const Ituple
               ituple_jjj;

               const Tuple
               tuple_p;

               void
               sum_up (Ncep_Ncar& ncep_ncar,
                       const Integer tolerance);

               void
               divide ();

            public:

               Work_Data_Map (const Ituple& ituple_jjj,
                              const Tuple& tuple_p,
                              const Tuple& tuple_latitude,
                              const Tuple& tuple_longitude);

               ~Work_Data_Map ();

               void
               calculate (Ncep_Ncar& ncep_ncar,
                          const Integer tolerance);

               void
               write (const string& path) const;

         };

         Integer
         nominal_year;

         Ituple
         ituple_jjj;

         Integer
         tolerance;

         Integer
         get_nearest_jjj (const Integer jjj) const;

         string
         get_file_path (const Integer jjj) const;

         void
         read_3d (Nwp::Data_3D& data_3d,
                  const Integer jjj) const;

         void
         write (const Work_Data_Map& work_data_map) const;

         Geodetic_Vector_Data_2D*
         get_initialized_vd_2d (const Integer vector_size) const;

         void
         initialize_3d_data (const Key& key);

         void
         load_3d_data (Nwp::Data_3D& data_3d);

         void
         fill_screen_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                 const Integer vector_index,
                                 const Key& key,
                                 const Nwp_Element nwp_element);

         void
         fill_10m_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                              const Integer vector_index,
                              const Key& key,
                              const Nwp_Element nwp_element);

         void
         fill_msl_data (Geodetic_Vector_Data_2D& gvd_2d,
                                  const Integer vector_index,
                                  const Key& key,
                                  const Nwp_Element nwp_element);

         void
         fill_surface_level_data (Geodetic_Vector_Data_2D& gvd_2d,
                                  const Integer vector_index,
                                  const Key& key,
                                  const Nwp_Element nwp_element);

      public:

         Ncep_Ncar_Climate (const string& description, 
                            const string& path,
                            const Integer nominal_year = -999);

         Ncep_Ncar_Climate (Ncep_Ncar& ncep_ncar,
                            const string& path,
                            const Ituple ituple_jjj,
                            const Integer tolerance,
                            const Tuple& tuple_p = Tuple ("100e2:200e2:300e2:400e2:500e2:600e2:700e2:850e2:925e2:1000e2"),
                            const Integer nominal_yeaer = -999);

         ~Ncep_Ncar_Climate ();

         void
         set_nominal_year (const Integer nominal_year);

         void
         survey ();

         void
         clean_up ();

         set<Dtime>
         get_valid_time_set () const;

         Key
         get_key (const Dtime& dtime) const;

         void
         acquire_base_time_forecast_hour (Dtime& base_time,
                                          Integer& forecast_hour,
                                          const Dtime& dtime) const;

   };

};

#endif /* DENISE_NWP_H */ 
